...
def run():
    ...
    print(f"Found {len(existing_db)} existing Aadhaar records in DB.")
    ...
    for r in range(2, sheet.max_row + 1):
        ...
        if aadhaar:
            if aadhaar in existing_db:
                skipped_db_dup += 1
                continue
            if aadhaar in seen_aadhaars_in_file:
                skipped_file_dup += 1
                continue
            seen_aadhaars_in_file.add(aadhaar)
        ...
        row_payload = {
            ...
            "saf_number": clean_str(get_val("SAF ID")) or None,
    ...
    print(f"\nSUCCESS! Successfully dumped {inserted_count} records to Supabase registrations table.")
