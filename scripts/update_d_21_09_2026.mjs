...
async function runUpdate() {
  console.log("=================================================================");
  console.log("=== Processing D-21.09.2026.xlsx: Update to 'Sent to Department' ===");
  console.log("=================================================================");

  // 1. Authenticate with an admin session
  const authEmail = `updater_d21_${Date.now()}@gleamator.com`;
  ...
  const excelFilePath = path.resolve(__dirname, "../public/D-21.09.2026.xlsx");
  ...
  console.log(`Extracted ${excelRecords.length} records from D-21.09.2026.xlsx.`);
  ...
  for (let i = 0; i < excelRecords.length; i++) {
    ...
    if (updateErr) {
      ...
      updatedCount++;
      console.log(`[${updatedCount}/${excelRecords.length}] Updated ${dbRow.reference_number} (${dbRow.saf_number}) -> 'Sent to Department'`);
    } else {
      errors.push({
        ref: dbRow.reference_number,
        aadhaar: item.aadhaar,
  ...
  console.log("Live DB Status Breakdown for all 31 records:", liveCounts);

  // Print all verified records
  console.log("\nComplete Verified Records Status in Live Database:");
  console.table(
    verifyList.map((r, idx) => ({
      "#": idx + 1,
      Reference: r.reference_number,
      SAF: r.saf_number,
      Name: `${r.first_name || ''} ${r.last_name || ''}`.trim(),
      Aadhaar: `XXXX-XXXX-${r.aadhaar_number.slice(-4)}`,
      Status: r.status,
      Notes: r.admin_notes,
    }))
  );
