...
async function runUpdate() {
  console.log("=== Processing V-10.09.2026 (1).xlsx: Update Status to 'Sent to Department' ===");

  // 1. Authenticate with an admin session
  const authEmail = `updater_v_${Date.now()}@gleamator.com`;
  ...
  const excelFilePath = path.resolve(__dirname, "../public/V-10.09.2026 (1).xlsx");
  ...
  console.log(`Extracted ${excelRecords.length} student records from V-10.09.2026 (1).xlsx.`);
  ...
  console.log("Status Breakdown for all matched records:", liveCounts);
