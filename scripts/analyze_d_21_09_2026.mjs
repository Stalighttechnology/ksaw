...
async function analyze() {
  const filePath = "c:/Users/raghu/Desktop/ksaw/public/D-21.09.2026.xlsx";
  ...
  for (const sheetName of wb.SheetNames) {
    ...
    if (safMismatches.length > 0) {
      console.log("SAF Mismatches sample (up to 5):", safMismatches.slice(0, 5));
    }

    console.log(`\nName Comparisons: Matches=${nameMatches}, Potential Mismatches/Variations=${nameMismatches.length}`);
    if (nameMismatches.length > 0) {
      console.log("Name variations sample (up to 5):", nameMismatches.slice(0, 5));
    }
    ...
    if (otherStatusMatches.length > 0) {
      console.log("  Other statuses detail:", otherStatusMatches.map(m => ({ ref: m.dbRec.reference_number, status: m.currentStatus, aadhaar: m.ex.aadhaar })));
    }
    console.log(`- Not found in DB: ${notFoundInDb.length}`);
    if (notFoundInDb.length > 0) {
      console.log("  Not found sample:", notFoundInDb.slice(0, 5));
    }
  }
}
