...
async function runUpdate() {
  console.log("=================================================================");
  console.log("=== Processing V-21.09.2026.xlsx: Update to 'Sent to Department' ===");
  console.log("=================================================================");

  // 1. Authenticate with an admin session
  const authEmail = `updater_v21_${Date.now()}@gleamator.com`;
  ...
  const excelFilePath = path.resolve(__dirname, "../public/V-21.09.2026.xlsx");
  const wb = xlsx.readFile(excelFilePath);
  const sheet = wb.Sheets[wb.SheetNames[0]]; // 'Master Sheet'
  ...
  console.log(`Extracted ${excelRecords.length} records from V-21.09.2026.xlsx.`);
  ...
  // 4. Batch update verified Approved records to "Sent to Department"
  let updatedCount = 0;
  let skippedCount = 0;
  const errors = [];
  ...
  for (let i = 0; i < excelRecords.length; i++) {
    ...
    if (dbRow.status !== "Approved") {
      skippedCount++;
      console.log(`Skipping non-approved record: ${dbRow.reference_number} (${dbRow.saf_number}) - Status: '${dbRow.status}'`);
      continue;
    }
    ...
    if (updateErr) {
      ...
      updatedCount++;
      if (updatedCount % 25 === 0 || updatedCount === 106) {
        console.log(`Progress: ${updatedCount} / 106 updated...`);
      }
    } else {
      errors.push({
        ref: dbRow.reference_number,
  ...
  console.log(`Skipped (Preserved Non-Approved): ${skippedCount}`);
  ...
  console.log("Live DB Status Breakdown for all 107 records:", liveCounts);
}
